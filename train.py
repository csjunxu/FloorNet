...
def build_graph_pointnet(options, input_dict):
    ...
    with tf.device('/gpu:%s' % options.gpu_id[0]):
        ...
        x1 = slim.conv2d(x0, nChannels[1], (1, nChannels[0]), stride=1, activation_fn=nn.relu, padding='valid',
                         normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))

        x2 = slim.conv2d(x1, nChannels[2], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                         weights_regularizer=slim.l2_regularizer(1e-5))

        x3 = slim.conv2d(x2, nChannels[3], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                         weights_regularizer=slim.l2_regularizer(1e-5))

        x4 = slim.conv2d(x3, nChannels[4], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                         weights_regularizer=slim.l2_regularizer(1e-5))

        x5 = slim.conv2d(x4, nChannels[5], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                         weights_regularizer=slim.l2_regularizer(1e-5))
        ...
        x5_up = slim.conv2d(x_fc_up, 512, (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                            weights_regularizer=slim.l2_regularizer(1e-5))
        x4_up = slim.conv2d(x5_up, 256, (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                            weights_regularizer=slim.l2_regularizer(1e-5))
        x3_up = slim.conv2d(x4_up, 128, (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                            weights_regularizer=slim.l2_regularizer(1e-5))
        x2_up = slim.conv2d(x3_up, 128, (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                            weights_regularizer=slim.l2_regularizer(1e-5))
        ...
        x2_up = tf.maximum(
            tf.unsorted_segment_max(tf.reshape(x2_up, (-1, 128)), tf.reshape(pointcloud_indices_inp, (-1,)),
                                    num_segments=options.batchSize * sizes[0] * sizes[0], name="project"), 0)
        ...
        if False:
            pred_corner = slim.conv2d(x1_up, 64, [3, 3], stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                      weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_corner')
            pred_icon = slim.conv2d(x1_up, 64, [3, 3], stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_icon')
            pred_room = slim.conv2d(x1_up, 64, [3, 3], stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_room')

            pred_corner = slim.conv2d(pred_corner, NUM_CORNERS, (1, 1), stride=1, activation_fn=None,
                                      normalizer_fn=None, weights_regularizer=slim.l2_regularizer(1e-5))
            pred_icon = slim.conv2d(pred_icon, NUM_ICONS, (1, 1), stride=1, activation_fn=None, normalizer_fn=None,
                                    weights_regularizer=slim.l2_regularizer(1e-5))
            pred_room = slim.conv2d(pred_room, NUM_ROOMS, (1, 1), stride=1, activation_fn=None, normalizer_fn=None,
                                    weights_regularizer=slim.l2_regularizer(1e-5))
        else:
            pred_corner = slim.conv2d(x1_up, NUM_CORNERS, [3, 3], stride=1, activation_fn=None, normalizer_fn=None,
                                      weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_corner')
            pred_icon = slim.conv2d(x1_up, NUM_ICONS, [3, 3], stride=1, activation_fn=None, normalizer_fn=None,
                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_icon')
            pred_room = slim.conv2d(x1_up, NUM_ROOMS, [3, 3], stride=1, activation_fn=None, normalizer_fn=None,
                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_room')
            pass

        pred_dict = {'corner': pred_corner, 'icon': pred_icon, 'room': pred_room}
        x0_topdown = tf.unsorted_segment_sum(tf.reshape(x0, (-1, NUM_CHANNELS[0])),
                                             tf.reshape(pointcloud_indices_inp, (-1,)),
                                             num_segments=options.batchSize / len(options.gpu_id) * SIZES[0] * SIZES[
                                                 0]) / options.sumScale
...
def build_graph_image(options, input_dict):
    ...
    if len(options.gpu_id) > 1:
        img_features = [tf.split(features, len(options.gpu_id), axis=0) for k, features in
                        image_features_all.iteritems()]
    else:
    ...
    for i, img_feature in zip(options.gpu_id, img_features):
        with tf.device('/gpu:%s' % int(i)), tf.variable_scope('floorplan_net', reuse=reused), slim.arg_scope(
                [slim.model_variable, slim.variable], device='/cpu:0'):
            x5_up = img_feature[4]

            x4_up = slim.conv2d_transpose(x5_up, NUM_CHANNELS[4], [5, 5], stride=2, activation_fn=nn.relu,
                                          normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))

            x4_up = mergeFeatures([x4_up, img_feature[3]])

            x3_up = slim.conv2d_transpose(x4_up, NUM_CHANNELS[3], [5, 5], stride=2, activation_fn=nn.relu,
                                          normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
            # print(x3_up)

            x3_up = mergeFeatures([x3_up, img_feature[2]])

            x2_up = slim.conv2d_transpose(x3_up, NUM_CHANNELS[2], [5, 5], stride=2, activation_fn=nn.relu,
                                          normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))

            x2_up = mergeFeatures([x2_up, img_feature[1]])

            x1_up = slim.conv2d_transpose(x2_up, NUM_CHANNELS[1], [5, 5], stride=2, activation_fn=nn.relu,
                                          normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))

            x1_up = mergeFeatures([x1_up, img_feature[0]])

            if True:
                pred_corner = slim.conv2d_transpose(x1_up, NUM_CHANNELS[1], [5, 5], stride=2, activation_fn=nn.relu,
                                                    normalizer_fn=bn_func,
                                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_corner')
                pred_icon = slim.conv2d_transpose(x1_up, NUM_CHANNELS[1], [5, 5], stride=2, activation_fn=nn.relu,
                                                  normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5),
                                                  scope='pred_icon')
                pred_room = slim.conv2d_transpose(x1_up, NUM_CHANNELS[1], [5, 5], stride=2, activation_fn=nn.relu,
                                                  normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5),
                                                  scope='pred_room')
                ...
                pred_corner = slim.conv2d_transpose(x1_up, NUM_CORNERS, [5, 5], stride=2, activation_fn=None,
                                                    normalizer_fn=bn_func,
                                                    weights_regularizer=slim.l2_regularizer(1e-5), scope='pred_corner')
                pred_icon = slim.conv2d_transpose(x1_up, NUM_ICONS, [5, 5], stride=2, activation_fn=None,
                                                  normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5),
                                                  scope='pred_icon')
                pred_room = slim.conv2d_transpose(x1_up, NUM_ROOMS, [5, 5], stride=2, activation_fn=None,
                                                  normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5),
                                                  scope='pred_room')
                pass
...
def build_graph(options, input_dict):
    ...
    debug_dicts = []
    reused = False
    for i, pointcloud_inp, pointcloud_indices_inp, img_feature in zip(options.gpu_id, pointcloud_inps,
                                                                      pointcloud_indices_inps, img_features):
        # for i in range(1):
        with tf.device('/gpu:%s' % int(i)), tf.variable_scope('floorplan_net', reuse=reused), slim.arg_scope(
                [slim.model_variable, slim.variable], device='/cpu:0'):
            ...
            if 'd' in options.augmentation:
                ...
                keep_prob = 0.5
                pointcloud_inp = tf.nn.dropout(pointcloud_inp, keep_prob,
                                               noise_shape=[options.batchSize, NUM_POINTS, 1]) * keep_prob
                pointcloud_indices_inp = tf.cast(
                    tf.round(tf.nn.dropout(tf.cast(pointcloud_indices_inp, np.float32), keep_prob) * keep_prob),
                    tf.int32)
            ...
            if options.branches == '1':
                x1_down = slim.conv2d(x0_down, NUM_CHANNELS[1], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                x2_down = slim.conv2d(x1_down, NUM_CHANNELS[2], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                x3_down = slim.conv2d(x2_down, NUM_CHANNELS[3], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                x4_down = slim.conv2d(x3_down, NUM_CHANNELS[4], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                x5_down = slim.conv2d(x4_down, NUM_CHANNELS[5], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                x5_up = x5_down
            else:
                x1 = slim.conv2d(x0, NUM_CHANNELS[1], (1, NUM_CHANNELS[0]), stride=1, activation_fn=nn.relu,
                                 padding='valid', normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                if options.poolingTypes[0] == 's':
                    x1_topdown = tf.unsorted_segment_sum(tf.reshape(x1, (-1, NUM_CHANNELS[1])),
                                                         tf.reshape(indices_maps[1], (-1,)),
                                                         num_segments=options.batchSize / len(options.gpu_id) * SIZES[
                                                             1] * SIZES[1]) / min(options.sumScale * 4,
                                                                                  (options.sumScale - 1) * 10000 + 1)
                else:
                    x1_topdown = tf.maximum(tf.unsorted_segment_max(tf.reshape(x1, (-1, NUM_CHANNELS[1])),
                                                                    tf.reshape(indices_maps[1], (-1,)),
                                                                    num_segments=options.batchSize / len(
                                                                        options.gpu_id) * SIZES[1] * SIZES[1]), 0)
                    pass
                ...
                if '2' in options.branches:
                    x1_unproject = tf.reshape(
                        tf.gather(tf.reshape(x1_down, (-1, NUM_CHANNELS[1])), indices_maps[1], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                ...
                x2 = slim.conv2d(x1, NUM_CHANNELS[2], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
                if options.poolingTypes[1] == 's':
                    x2_topdown = tf.unsorted_segment_sum(tf.reshape(x2, (-1, NUM_CHANNELS[2])),
                                                         tf.reshape(indices_maps[2], (-1,)),
                                                         num_segments=options.batchSize / len(options.gpu_id) * SIZES[
                                                             2] * SIZES[2]) / min(options.sumScale * 16,
                                                                                  (options.sumScale - 1) * 10000 + 1)
                else:
                    x2_topdown = tf.maximum(tf.unsorted_segment_max(tf.reshape(x2, (-1, NUM_CHANNELS[2])),
                                                                    tf.reshape(indices_maps[2], (-1,)),
                                                                    num_segments=options.batchSize / len(
                                                                        options.gpu_id) * SIZES[2] * SIZES[2]), 0)
                    pass
                ...
                x2_down = slim.conv2d(x1_down, NUM_CHANNELS[2], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                if '2' in options.branches:
                    x2_unproject = tf.reshape(
                        tf.gather(tf.reshape(x2_down, (-1, NUM_CHANNELS[2])), indices_maps[2], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                ...
                x3 = slim.conv2d(x2, NUM_CHANNELS[3], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
                if options.poolingTypes[2] == 's':
                    x3_topdown = tf.unsorted_segment_sum(tf.reshape(x3, (-1, NUM_CHANNELS[3])),
                                                         tf.reshape(indices_maps[3], (-1,)),
                                                         num_segments=options.batchSize / len(options.gpu_id) * SIZES[
                                                             3] * SIZES[3]) / min(options.sumScale * 64,
                                                                                  (options.sumScale - 1) * 10000 + 1)
                else:
                    x3_topdown = tf.maximum(tf.unsorted_segment_max(tf.reshape(x3, (-1, NUM_CHANNELS[3])),
                                                                    tf.reshape(indices_maps[3], (-1,)),
                                                                    num_segments=options.batchSize / len(
                                                                        options.gpu_id) * SIZES[3] * SIZES[3]), 0)
                    pass
                ...
                x3_down = slim.conv2d(x2_down, NUM_CHANNELS[3], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                if '2' in options.branches:
                    x3_unproject = tf.reshape(
                        tf.gather(tf.reshape(x3_down, (-1, NUM_CHANNELS[3])), indices_maps[3], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                ...
                x4 = slim.conv2d(x3, NUM_CHANNELS[4], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
                if options.poolingTypes[3] == 's':
                    x4_topdown = tf.unsorted_segment_sum(tf.reshape(x4, (-1, NUM_CHANNELS[4])),
                                                         tf.reshape(indices_maps[4], (-1,)),
                                                         num_segments=options.batchSize / len(options.gpu_id) * SIZES[
                                                             4] * SIZES[4]) / min(options.sumScale * 256,
                                                                                  (options.sumScale - 1) * 10000 + 1)
                else:
                    x4_topdown = tf.maximum(tf.unsorted_segment_max(tf.reshape(x4, (-1, NUM_CHANNELS[4])),
                                                                    tf.reshape(indices_maps[4], (-1,)),
                                                                    num_segments=options.batchSize / len(
                                                                        options.gpu_id) * SIZES[4] * SIZES[4]), 0)
                    pass
                x4_topdown = tf.reshape(x4_topdown, (options.batchSize / len(options.gpu_id), SIZES[4], SIZES[4], -1))
                x4_down = slim.conv2d(x3_down, NUM_CHANNELS[4], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                if '2' in options.branches:
                    x4_unproject = tf.reshape(
                        tf.gather(tf.reshape(x4_down, (-1, NUM_CHANNELS[4])), indices_maps[4], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                ...
                x5 = slim.conv2d(x4, NUM_CHANNELS[5], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
                if options.poolingTypes[4] == 's':
                    x5_topdown = tf.unsorted_segment_sum(tf.reshape(x5, (-1, NUM_CHANNELS[5])),
                                                         tf.reshape(indices_maps[5], (-1,)),
                                                         num_segments=options.batchSize / len(options.gpu_id) * SIZES[
                                                             5] * SIZES[5]) / min(options.sumScale * 1024,
                                                                                  (options.sumScale - 1) * 10000 + 1)
                else:
                    x5_topdown = tf.maximum(tf.unsorted_segment_max(tf.reshape(x5, (-1, NUM_CHANNELS[5])),
                                                                    tf.reshape(indices_maps[5], (-1,)),
                                                                    num_segments=options.batchSize / len(
                                                                        options.gpu_id) * SIZES[5] * SIZES[5]), 0)
                    pass
                x5_topdown = tf.reshape(x5_topdown, (options.batchSize / len(options.gpu_id), SIZES[5], SIZES[5], -1))
                x5_down = slim.conv2d(x4_down, NUM_CHANNELS[5], (3, 3), stride=2, activation_fn=nn.relu,
                                      normalizer_fn=bn_func, weights_regularizer=slim.l2_regularizer(1e-5))
                if '2' in options.branches:
                    x5_unproject = tf.reshape(
                        tf.gather(tf.reshape(x5_down, (-1, NUM_CHANNELS[5])), indices_maps[5], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
            ...
            if '3' in options.branches:
                ...
                x5 = slim.conv2d(x_fc, NUM_CHANNELS[5], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
            ...
            if '3' in options.branches:
                x4 = slim.conv2d(x5, NUM_CHANNELS[4], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))
                if '2' in options.branches:
                    x4_up_unproject = tf.reshape(
                        tf.gather(tf.reshape(x4_up, (-1, NUM_CHANNELS[4])), indices_maps[4], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                    # x4 = tf.add(x4, tf.expand_dims(x4_up_unproject, 2))
                    x4 = mergeFeatures([x4, tf.expand_dims(x4_up_unproject, 2)])
                    pass

                x4_up_topdown = tf.maximum(
                    tf.unsorted_segment_max(tf.reshape(x4, (-1, NUM_CHANNELS[4])), tf.reshape(indices_maps[4], (-1,)),
                                            num_segments=options.batchSize / len(options.gpu_id) * SIZES[4] * SIZES[4]),
                    0)
                x4_up_topdown = tf.reshape(x4_up_topdown,
                                           (options.batchSize / len(options.gpu_id), SIZES[4], SIZES[4], -1))
                if '0' in options.branches:
            ...
            if '3' in options.branches:
                x3 = slim.conv2d(x4, NUM_CHANNELS[3], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))

                if '2' in options.branches:
                    x3_up_unproject = tf.reshape(
                        tf.gather(tf.reshape(x3_up, (-1, NUM_CHANNELS[3])), indices_maps[3], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                    # x3 = tf.add(x3, tf.expand_dims(x3_up_unproject, 2))
                    x3 = mergeFeatures([x3, tf.expand_dims(x3_up_unproject, 2)])
                    pass

                x3_up_topdown = tf.maximum(
                    tf.unsorted_segment_max(tf.reshape(x3, (-1, NUM_CHANNELS[3])), tf.reshape(indices_maps[3], (-1,)),
                                            num_segments=options.batchSize / len(options.gpu_id) * SIZES[3] * SIZES[3]),
                    0)
                x3_up_topdown = tf.reshape(x3_up_topdown,
                                           (options.batchSize / len(options.gpu_id), SIZES[3], SIZES[3], -1))
                if '0' in options.branches:
            ...
            if '3' in options.branches:
                x2 = slim.conv2d(x3, NUM_CHANNELS[2], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))

                if '2' in options.branches:
                    x2_up_unproject = tf.reshape(
                        tf.gather(tf.reshape(x2_up, (-1, NUM_CHANNELS[2])), indices_maps[2], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                    # x2 = tf.add(x2, tf.expand_dims(x2_up_unproject, 2))
                    x2 = mergeFeatures([x2, tf.expand_dims(x2_up_unproject, 2)])
                    pass

                x2_up_topdown = tf.maximum(
                    tf.unsorted_segment_max(tf.reshape(x2, (-1, NUM_CHANNELS[2])), tf.reshape(indices_maps[2], (-1,)),
                                            num_segments=options.batchSize / len(options.gpu_id) * SIZES[2] * SIZES[2]),
                    0)
                x2_up_topdown = tf.reshape(x2_up_topdown,
                                           (options.batchSize / len(options.gpu_id), SIZES[2], SIZES[2], -1))
                if '0' in options.branches:
            ...
            if '3' in options.branches:
                x1 = slim.conv2d(x2, NUM_CHANNELS[1], (1, 1), stride=1, activation_fn=nn.relu, normalizer_fn=bn_func,
                                 weights_regularizer=slim.l2_regularizer(1e-5))

                if '2' in options.branches:
                    x1_up_unproject = tf.reshape(
                        tf.gather(tf.reshape(x1_up, (-1, NUM_CHANNELS[1])), indices_maps[1], validate_indices=False),
                        (options.batchSize / len(options.gpu_id), options.numPoints, -1))
                ...
                x1_up_topdown = tf.maximum(
                    tf.unsorted_segment_max(tf.reshape(x1, (-1, NUM_CHANNELS[1])), tf.reshape(indices_maps[1], (-1,)),
                                            num_segments=options.batchSize / len(options.gpu_id) * SIZES[1] * SIZES[1]),
                    0)
                x1_up_topdown = tf.reshape(x1_up_topdown,
                                           (options.batchSize / len(options.gpu_id), SIZES[1], SIZES[1], -1))
...
def build_loss(options, pred_dict, gt_dict, dataset_flag, debug_dict, flags=None):
    with tf.device('/gpu:0'):
        ...
        cornerSegmentation = tf.stack([tf.sparse_to_dense(
            tf.stack([corners[batchIndex, :, 1], corners[batchIndex, :, 0]], axis=1), (HEIGHT, WIDTH),
            corners[batchIndex, :, 2], validate_indices=False) for batchIndex in xrange(options.batchSize)], axis=0)
        ...
        neighbor_kernel = tf.constant(neighbor_kernel_array.reshape(-1), shape=neighbor_kernel_array.shape,
                                      dtype=tf.float32)
        neighbor_kernel = tf.reshape(neighbor_kernel, [kernel_size, kernel_size, 1, 1])
        cornerHeatmaps = tf.nn.depthwise_conv2d(cornerHeatmaps, tf.tile(neighbor_kernel, [1, 1, NUM_CORNERS, 1]),
        ...
        if options.cornerLossType == 'softmax':
            corner_loss = tf.reduce_mean(
                tf.losses.sparse_softmax_cross_entropy(logits=pred_dict['corner'], labels=corner_gt,
                                                       weights=tf.maximum(tf.cast(corner_gt > 0, tf.float32) * 100, 1)),
                axis=[0, 1, 2])
            ...
            corner_gt = tf.nn.depthwise_conv2d(cornerHeatmaps, tf.tile(neighbor_kernel, [1, 1, NUM_CORNERS, 1]),
                                               strides=[1, 1, 1, 1], padding='SAME')
            ...
            corner_loss = tf.reduce_mean(
                tf.losses.sigmoid_cross_entropy(logits=pred_dict['corner'], multi_class_labels=corner_gt,
                                                weights=tf.maximum(tf.cast(corner_gt > 0.5, tf.float32) * 5, 1),
                                                reduction=tf.losses.Reduction.NONE), axis=[0, 1, 2])
            pass

        # dataset_flag = flags[0][0]
        corner_loss = tf.reduce_mean(corner_loss * corner_valid_masks[dataset_flag])

        icon_loss = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(logits=pred_dict['icon'], labels=icon_gt,
                                                                          weights=tf.maximum(tf.cast(icon_gt > 0,
                                                                                                     tf.float32) * options.iconPositiveWeight,
                                                                                             1),
                                                                          reduction=tf.losses.Reduction.NONE),
                                   axis=[0, 1, 2])
        icon_loss = tf.reduce_mean(icon_loss * icon_valid_masks[dataset_flag])
        room_loss = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(logits=pred_dict['room'], labels=room_gt,
                                                                          reduction=tf.losses.Reduction.NONE),
                                   axis=[0, 1, 2])
        ...
        if options.branches == '4':
            corner_loss = tf.reduce_mean(
                tf.losses.sigmoid_cross_entropy(logits=pred_dict['corner'], multi_class_labels=corner_gt,
                                                weights=tf.maximum(tf.cast(corner_gt > 0.5, tf.float32) * 5, 1),
                                                reduction=tf.losses.Reduction.NONE), axis=[1, 2, 3])
            corner_loss = tf.reduce_sum(corner_loss * tf.cast(flags[:, 1], tf.float32))

            icon_loss = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(logits=pred_dict['icon'], labels=icon_gt,
                                                                              weights=tf.maximum(
                                                                                  tf.cast(icon_gt > 0, tf.float32) * 10,
                                                                                  1),
                                                                              reduction=tf.losses.Reduction.NONE),
                                       axis=[1, 2])
...
def train(options):
    ...
    if options.slice:
        ...
        dataset_train = RecordReaderSlice.getDatasetTrain(filenames_train, options.augmentation,
                                                          '4' in options.branches, options.batchSize)
    else:
        dataset_train = getDatasetTrain(filenames_train, options.augmentation, '4' in options.branches,
                                        options.batchSize)
    ...
    with tf.Session(config=config) as sess:
        ...
        while bno < options.numIterations * (6.0 / options.batchSize):
            ...
            try:
                if profileTime:
                    for iteration in xrange(5):
                        ...
                        # else:
                        _, total_loss, losses, summary_str, dataset, gt, pred, debug = sess.run(
                            [optimizer, loss, loss_list, summary_op, dataset_flag, gt_dict, pred_dict, debug_dict],
                            feed_dict={handle: handle_train}, run_metadata=run_metadata, options=run_options)
                ...
                for iteration in xrange(500):
                    ...
                    if iteration == 250 and options.visualize:
                        ...
                        visualizeBatch(options, 'train', pred,
                                       {'corner': gt['corner'], 'icon': gt['icon'], 'room': gt['room'],
                                        'density': debug['x0_topdown'][:, :, :, -1]}, dataset)
                        # print(dataset)
                        # print(losses)
                        ...
                        _, total_loss, losses, summary_str, dataset = sess.run(
                            [optimizer, loss, loss_list, summary_op, dataset_flag], feed_dict={handle: handle_train})
                        pass
                    ...
                    print('%d: train- %02f %02f %02f, val- %02f %02f %02f %02f' % (
                    bno + iteration, train_losses[0] / train_acc[0], train_losses[1] / train_acc[1],
                    train_losses[2] / train_acc[2], val_losses[0] / val_acc[0], val_losses[1] / val_acc[1],
                    val_losses[2] / val_acc[2], time.time() - t0))
            ...
            try:
                ...
                for iteration in xrange(10):
                    if iteration == 0 and options.visualize:
                        total_loss, losses, summary_str, dataset, gt, pred, debug = sess.run(
                            [loss, loss_list, summary_op, dataset_flag, gt_dict, pred_dict, debug_dict],
                            feed_dict={handle: handle_val})
                        visualizeBatch(options, 'val', pred,
                                       {'corner': gt['corner'], 'icon': gt['icon'], 'room': gt['room'],
                                        'density': debug['x0_topdown'][:, :, :, -1]}, dataset)
                    else:
                        total_loss, losses, summary_str, dataset = sess.run([loss, loss_list, summary_op, dataset_flag],
                                                                            feed_dict={handle: handle_val})
                        pass
                    for lossIndex, value in enumerate(losses):
                        ...
                        continue
                    print('validation', 't', train_losses[0] / train_acc[0], train_losses[1] / train_acc[1],
                          train_losses[2] / train_acc[2], 'v', val_losses[0] / val_acc[0], val_losses[1] / val_acc[1],
                          val_losses[2] / val_acc[2])
            ...
            print(
            bno, 't', train_losses[0] / train_acc[0], train_losses[1] / train_acc[1], train_losses[2] / train_acc[2],
            'v', val_losses[0] / val_acc[0], val_losses[1] / val_acc[1], val_losses[2] / val_acc[2])
            continue
        pass
    return
...
def test(options):
    ...
    with tf.Session(config=config) as sess:
            ...
            for iteration in xrange(options.numTestingImages):
                ...
                for batchIndex in xrange(options.batchSize):
                    ...
                    for cornerType, ranges in CORNER_RANGES.iteritems():
                        if options.cornerLossType == "mse":
                            cornersPred = extractCornersFromHeatmaps(cornerHeatmapsPred[:, :, ranges[0]:ranges[1]],
                                                                     heatmapThreshold=0.5, returnRanges=False)
                        else:
                            cornersPred = extractCornersFromHeatmaps(cornerHeatmapsPred[:, :, ranges[0]:ranges[1]],
                                                                     returnRanges=False)
                            pass
                        cornersGT = extractCornersFromHeatmaps(cornerHeatmapsGT[:, :, ranges[0]:ranges[1]],
                                                               heatmapThreshold=0.5, returnRanges=False)
                    ...
                    iconStatistics = evaluateSegmentation(np.argmax(pred['icon'][batchIndex], axis=-1),
                                                          gt['icon'][batchIndex])
                    iconCounter += iconStatistics

                    roomStatistics = evaluateSegmentation(np.argmax(pred['room'][batchIndex], axis=-1),
                                                          gt['room'][batchIndex])
                ...
                statistics = visualizeBatch(options, 'test_' + str(iteration), pred,
                                            {'corner': gt['corner'], 'icon': gt['icon'], 'room': gt['room'],
                                             ...
                                             'image_path': inp['image_path']}, dataset)
        ...
        for cornerType, cornerCounter in cornerCounters.iteritems():
            print(cornerType + ' corner recall and precision', cornerCounter[0] / cornerCounter[1],
                  cornerCounter[0] / cornerCounter[2])
            continue
        ...
        print('precision and recall',
              [(k, float(v[0]) / max(v[1], 1), float(v[0]) / max(v[2], 1)) for k, v in statisticsSum.iteritems()])
        ...
        numbers['opening_corner'] = (
        cornerCounters['opening'][0], cornerCounters['opening'][1], cornerCounters['opening'][2])
...
def visualizeBatch(options, prefix, pred_dict, gt_dict, datasetFlag):
    ...
    for batchIndex in xrange(options.batchSize):
        ...
        if datasetFlag in [1, 4]:
            ...
            cornerImage[cornerImage == 0] = density[cornerImage == 0]
            cv2.imwrite(options.test_dir + '/' + prefix + '_' + str(batchIndex) + '_opening_corner_pred.png',
                        cornerImage)
        ...
        if batchIndex == 0 and False:
            for c in xrange(22):
                cv2.imwrite(options.test_dir + '/mask_' + str(c) + '.png',
                            cv2.dilate(drawMaskImage(corner_segmentation[batchIndex] == c), np.ones((3, 3)), 3))
                continue
            continue
        ...
        if batchIndex < options.visualizeReconstruction:
            ...
            try:
                ...
                for heatmapIndex in xrange(gtHeatmaps.shape[-1]):
                    ...
                    continue
                result_gt = reconstructFloorplan(gtHeatmaps[:, :, :NUM_WALL_CORNERS],
                                                 gtHeatmaps[:, :, NUM_WALL_CORNERS:NUM_WALL_CORNERS + 4],
                                                 gtHeatmaps[:, :, NUM_WALL_CORNERS + 4:NUM_WALL_CORNERS + 8],
                ...
                cv2.imwrite(options.test_dir + '/' + prefix + '_' + str(batchIndex) + '_reconstruction_gt.png',
                            resultImage)
                # if batchIndex == 1:
                # exit(1)
                ...
                result_pred = reconstructFloorplan(pred_wc[batchIndex], pred_oc[batchIndex], pred_ic[batchIndex],
                                                   sigmoid(pred_dict['icon'][batchIndex]),
                                                   sigmoid(pred_dict['room'][batchIndex]), density[:, :, 0],
                                                   gt_dict=gt_dict, gap=5, distanceThreshold=5, lengthThreshold=10,
                                                   gt=False, debug_prefix=pred_debug_dir)
                # print(result_pred)
                if len(result_pred) == 0:
                    continue
                resultImage = drawResultImage(WIDTH, HEIGHT, result_pred)
                cv2.imwrite(options.test_dir + '/' + prefix + '_' + str(batchIndex) + '_reconstruction_pred.png',
                            resultImage)
