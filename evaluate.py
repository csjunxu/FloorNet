...
def evaluate(options):
	...
	if os.path.exists(options.test_dir + '/dummy'):
		...
		pass
	pdb.set_trace()
	if options.useCache == 2 and os.path.exists(options.test_dir + '/dummy/gt_dict.npy') and os.path.exists(
			options.test_dir + '/dummy/pred_dict.npy'):
		return

	if options.useCache == 1 and os.path.exists(options.test_dir + '/dummy/gt_dict.npy') and os.path.exists(
			options.test_dir + '/dummy/pred_dict.npy'):
...
def evaluateBatch(options, gt_dict=None, pred_dict=None):
	...
	if options.useCache != -1:
		if options.loss != '5':
			...
			pred_wc = np.load(options.test_dir.replace('loss5', 'loss0') + '/dummy/pred_dict.npy')[()]['corner'][:, :,
					  :, :NUM_WALL_CORNERS]
			pred_oc = np.load(options.test_dir.replace('loss5', 'loss1') + '/dummy/pred_dict.npy')[()]['corner'][:, :,
					  :, NUM_WALL_CORNERS:NUM_WALL_CORNERS + 4]
			pred_ic = \
			np.load(options.test_dir.replace('loss5', 'loss2').replace('hybrid14', 'hybrid1') + '/dummy/pred_dict.npy')[
				()]['corner'][:, :, :, NUM_WALL_CORNERS + 4:NUM_WALL_CORNERS + 8]
			pred_icon = \
			np.load(options.test_dir.replace('loss5', 'loss3').replace('hybrid14', 'hybrid1') + '/dummy/pred_dict.npy')[
				()]['icon']
			pred_room = \
			np.load(options.test_dir.replace('loss5', 'loss4').replace('hybrid14', 'hybrid1') + '/dummy/pred_dict.npy')[
				()]['room']
			pred_dict = {'corner': np.concatenate([pred_wc, pred_oc, pred_ic], axis=-1), 'icon': pred_icon,
						 'room': pred_room}
		pass

	if options.separateIconLoss:
		pred_icon_separate = softmax(
			np.load(options.test_dir.replace('wsf', 'wsf_loss3') + '/dummy/pred_dict.npy')[()]['icon'])
		pass
	...
	for batchIndex in xrange(gt_dict['corner'].shape[0]):
		...
		if True:
			...
			if datasetFlag in [0, 1, 4]:
				cornerImage = drawSegmentationImage(np.concatenate([threshold, pred_wc[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
				cv2.imwrite(options.test_dir + '/' + str(batchIndex) + '_corner_pred.png', cornerImage)

				cornerImage = drawSegmentationImage(np.concatenate([threshold, gt_wc[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
			...
			if datasetFlag in [1, 4]:
				cornerImage = drawSegmentationImage(np.concatenate([threshold, pred_oc[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
				cv2.imwrite(options.test_dir + '/' + str(batchIndex) + '_opening_corner_pred.png', cornerImage)

				cornerImage = drawSegmentationImage(np.concatenate([threshold, pred_ic[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
				cv2.imwrite(options.test_dir + '/' + str(batchIndex) + '_icon_corner_pred.png', cornerImage)

				cornerImage = drawSegmentationImage(np.concatenate([threshold, gt_oc[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
				cv2.imwrite(options.test_dir + '/' + str(batchIndex) + '_opening_corner_gt.png', cornerImage)

				cornerImage = drawSegmentationImage(np.concatenate([threshold, gt_ic[batchIndex]], axis=2),
													blackIndex=0)
				cornerImage[cornerImage == 0] = density[cornerImage == 0]
