...
GAPS = {'wall_extraction': 5, 'door_extraction': 5, 'icon_extraction': 5, 'wall_neighbor': 5, 'door_neighbor': 5,
        'icon_neighbor': 5, 'wall_conflict': 5, 'door_conflict': 5, 'icon_conflict': 5, 'wall_icon_neighbor': 5,
        'wall_icon_conflict': 5, 'wall_door_neighbor': 5, 'door_point_conflict': 5}
...
def findMatches(pred_dict, gt_dict, distanceThreshold, width=256, height=256):
            ...
            if True:
                ...
                for pointIndexGT, pointGT in enumerate(validPointsGT):
                    ...
                    for pointIndexPred, pointPred in enumerate(validPointsPred):
                        if pointPred[2] == pointGT[2] and pointPred[3] == pointGT[3] and pointDistance(pointPred[0:2],
                                                                                                       pointGT[
                                                                                                       0:2]) < distanceThreshold:
                ...
                for pointIndexGT, pointGT in enumerate(validPointsGT):
                    ...
                    for orientation, hasMatch in matchedOrientations.iteritems():
                        ...
                        continue
                    numMatches += len(
                        [orientation for orientation, value in matchedOrientations.iteritems() if value == True])
                    continue
        ...
        if objectType == 'door':
            ...
            for lineIndexGT, lineGT in enumerate(linesGT):
                ...
                for lineIndexPred, linePred in enumerate(linesPred):
                    ...
                    if (pointDistance(pointsPred[linePred[0]],
                                      pointsGT[lineGT[0]]) < distanceThreshold and pointDistance(
                            pointsPred[linePred[1]], pointsGT[lineGT[1]]) < distanceThreshold) or (
                            pointDistance(pointsPred[linePred[0]],
                                          pointsGT[lineGT[1]]) < distanceThreshold and pointDistance(
                            pointsPred[linePred[1]], pointsGT[lineGT[0]]) < distanceThreshold):
        ...
        if objectType == 'icon':
            ...
            for indexGT, rectangleGT in enumerate(rectanglesGT):
                ...
                for indexPred, rectanglePred in enumerate(rectanglesPred):
                    if labelsGT[indexGT] == labelsPred[indexPred] and calcIOU(
                            [pointsPred[pointIndex] for pointIndex in rectanglePred],
                            [pointsGT[pointIndex] for pointIndex in rectangleGT]) >= 0.3:
                        matchedRectangleMask[indexPred] = True
                        pass
                    continue
                if len(matchedRectangleMask) == 0:
                    print(
                    indexGT, rectangleGT, [pointsGT[pointIndex][:2] for pointIndex in rectangleGT], 'icon not found')
                    pass
    ...
    for dictIndex in xrange(2):
        ...
        wallMask = drawWallMask([(wall_dict[0][line[0]], wall_dict[0][line[1]]) for line in wall_dict[1]], width,
                                height, thickness=wallLineWidth)
        ...
        for doorLine in door_dict[1]:
            ...
            center = np.round(
                (np.array(door_dict[0][doorLine[0]][:2]) + np.array(door_dict[0][doorLine[1]][:2])) / 2).astype(
                np.int32)
            neighborRoomPair = []
            for c in xrange(2):
                direction = c * 2 - 1
        ...
        neighborRoomPairs = [(indexMap[neighborRoomPair[0]], indexMap[neighborRoomPair[1]]) for neighborRoomPair in
                             neighborRoomPairs]
        ...
        adjacentRoomPairs = [(indexMap[adjacentRoomPair[0]], indexMap[adjacentRoomPair[1]]) for adjacentRoomPair in
                             adjacentRoomPairs]
    ...
    for roomIndexPred, roomPred in enumerate(roomsInfo[1][0]):
        ...
        if roomIndexGT not in orderedRoomPred:
            ...
            pass
        continue
    roomIndexMap = (np.expand_dims(roomIndexMap, -1) == np.expand_dims(np.arange(len(roomsInfo[0][0]), dtype=np.int32),
                                                                       0)).astype(np.int32)
    ...
    topologyStatistics = {k: [0.0, 0.0, 0.0] for k in
                          ['adjacent', 'neighbor', 'neighbor_foreground', 'adjacent_all', 'neighbor_all',
                           'neighbor_all_foreground', 'all', 'all_foreground']}
    ...
    for roomIndex, roomGT in enumerate(roomsInfo[0][0]):
        ...
        if hasCommonLabel and calcIOUMask(roomPred[0], roomGT[0]) >= 0.5:
            ...
            if roomIndex > 0:
                topologyStatistics['neighbor_foreground'][0] += (
                            neighborMatchMask[1:] * roomsInfo[0][1][roomIndex][1:]).sum()
                ...
                topologyStatistics['all_foreground'][0] += int(
                    np.all(neighborMatchMask[1:]) and np.all(adjacentMatchMask[1:]))
                pass
        else:
            ...
            for cornerIndex, corner in enumerate(corners):
                ...
                for oppositeOrientationIndex, oppositeCorners in enumerate(orientationPoints):
                    ...
                    for oppositeCornerIndex, oppositeCorner in enumerate(oppositeCorners):
                        ...
                        if oppositeCorner[0][lineDim] < ranges[lineDim] or oppositeCorner[0][lineDim] > ranges[
                            lineDim + 2] or ranges[1 - lineDim] > oppositeCorner[2][1 - lineDim] or ranges[
                            1 - lineDim + 2] < oppositeCorner[1][1 - lineDim]:
                            continue
                ...
                for cornerIndex2, corner2 in enumerate(corners2):
                    ...
                    for orientationIndex3, corners3 in enumerate(orientationPoints):
                        for cornerIndex3, corner3 in enumerate(corners3):
                            ...
                            if pointIndex2 in pointNeighbors[pointIndex1] and pointIndex3 in pointNeighbors[
                                pointIndex2]:
                                ...
                                for point4 in fourthPoints:
                                    if abs(points[point4][0] - point2[0]) > gap and abs(
                                            points[point4][1] - point2[1]) > gap:
                                ...
                                if len(valid_fourth) == 0:
                                    ...
                                    print('test orientation', oppositeOrientation2, oppositeOrientation1,
                                          oppositeOrientation3)
                                    newPoint1 = [point1[0], point3[1], newPoint_orientation / 4,
                                                 newPoint_orientation % 4]
                                    newPoint2 = [point3[0], point1[1], newPoint_orientation / 4,
                                                 newPoint_orientation % 4]
                                    verify11 = myVerifyCompatibility(oppositeOrientation1, oppositeOrientation2, point1,
                                                                     newPoint1, gap)
                                    verify31 = myVerifyCompatibility(oppositeOrientation3, oppositeOrientation2, point3,
                                                                     newPoint1, gap)
                                    verify12 = myVerifyCompatibility(oppositeOrientation1, oppositeOrientation2, point1,
                                                                     newPoint2, gap)
                                    verify32 = myVerifyCompatibility(oppositeOrientation3, oppositeOrientation2, point3,
                                                                     newPoint2, gap)
                                    if abs(newPoint1[0] - point2[0]) > gap and abs(
                                            newPoint1[1] - point2[1]) > gap and verify11 and verify31:
                                        if newPoint_orientation not in augmented_points:
                                            augmented_points[newPoint_orientation] = []
                                        print('case1', newPoint1, point1, point2, point3, abs(newPoint1[0] - point2[0]),
                                              abs(newPoint1[1] - point2[1]))
                                        augmented_points[newPoint_orientation].append(((newPoint1[0], newPoint1[1]), (
                                        newPoint1[0] - gap, newPoint1[1] - gap), (newPoint1[0] + gap,
                                                                                  newPoint1[1] + gap), True))
                                        pass
                                    elif verify12 and verify32:
                                        if newPoint_orientation not in augmented_points:
                                            augmented_points[newPoint_orientation] = []
                                            pass
                                        print('case2', newPoint2, point1, point2, point3, abs(newPoint2[0] - point2[0]),
                                              abs(newPoint2[1] - point2[1]))
                                        augmented_points[newPoint_orientation].append(((newPoint2[0], newPoint2[1]), (
                                        newPoint2[0] - gap, newPoint2[1] - gap), (newPoint2[0] + gap,
                                                                                  newPoint2[1] + gap), True))
                                        pass
    ...
    for pointIndex, point in enumerate(points):
        ...
        for orientation in xrange(4):
            ...
            for neighborPointIndex, neighborPoint in enumerate(points):
                ...
                if not inRange or abs(neighborPoint[lineDim] - point[lineDim]) < max(
                        abs(neighborPoint[1 - lineDim] - point[1 - lineDim]), 1):
                    continue
...
def filterWallsDynamic(wallPoints, wallLines):
    ...
    while True:
        ...
        image = drawLines('', width, height, wallPoints, wallLines, [], None, lineWidth=0,
                          lineColor=np.array([0, 0, 128]))
        ...
        for pointIndex, _ in invalidPointMask.iteritems():
            newImage = image.copy()
            newImage = drawPoints('', width, height, [wallPoints[pointIndex]], newImage, pointSize=3,
                                  pointColor=np.array([255, 0, 255]))
            connectingLines = []
            for line in wallLines:
            ...
            newImage = drawLines('', width, height, wallPoints, connectingLines, [], newImage, 1,
    ...
    with open('test/point_labels.txt', 'w') as point_label_file:
        for point in pointLabels:
            point_label_file.write(
                str(point[0]) + '\t' + str(point[1]) + '\t' + str(point[2]) + '\t' + str(point[3]) + '\n')
...
def adjustPoints(points, lines):
    ...
    for lineIndex, line in enumerate(lines):
        ...
        for neighborLineIndex, neighborLine in enumerate(lines):
            ...
            if lineDimNeighbor != lineDim:
                continue
            if neighborLine[0] != line[0] and neighborLine[0] != line[1] and neighborLine[1] != line[0] and \
                    neighborLine[1] != line[1]:
                continue
    ...
    for lineIndex in xrange(len(lines)):
        ...
        xy = np.concatenate(
            [np.array([points[pointIndex][:2] for pointIndex in lines[lineIndex]]) for lineIndex in lineGroup], axis=0)
        mins = xy.min(0)
        maxs = xy.max(0)
    ...
    for pointIndex_1, point_1 in enumerate(points):
        ...
        for pointIndex_2, point_2 in enumerate(points):
            ...
            if pointDistance(point_1[:2], point_2[:2]) <= DISTANCES['point']:
                orientations = list(
                    POINT_ORIENTATIONS[point_1[2]][point_1[3]] + POINT_ORIENTATIONS[point_2[2]][point_2[3]])
                ...
                if orientations not in orientationMap:
                    ...
                    lineIndices_1 = [(lineIndex, tuple(set(line) - set((pointIndex_1,)))[0]) for lineIndex, line in
                                     enumerate(lines) if pointIndex_1 in line and pointIndex_2 not in line]
                    lineIndices_2 = [(lineIndex, tuple(set(line) - set((pointIndex_2,)))[0]) for lineIndex, line in
                                     enumerate(lines) if pointIndex_2 in line and pointIndex_1 not in line]
        ...
        for lineIndex, neighbors in enumerate(lineNeighbors):
            ...
            for neighbor_1, region_1 in neighbors[1].iteritems():
                for neighbor_2, _ in neighbors[0].iteritems():
                    ...
                    if region_1[0][lineDim] < region_2[0][lineDim] + gap and region_1[1][lineDim] > region_2[1][
                        lineDim] - gap:
    ...
    for lineIndex, directionNeighbors in enumerate(lineNeighbors):
        for direction, neighbors in enumerate(directionNeighbors):
            for neighbor, region in neighbors.iteritems():
                labelVotes = labelVotesMap[:, region[1][1], region[1][0]] + labelVotesMap[:, region[0][1],
                                                                            region[0][0]] - labelVotesMap[:,
                                                                                            region[0][1], region[1][
                                                                                                              0]] - labelVotesMap[
                                                                                                                    :,
                                                                                                                    region[
...
def findRectangleLineNeighbors(rectanglePoints, rectangles, linePoints, lines, lineNeighbors, gap, distanceThreshold):
    ...
    for rectangleIndex, rectangle in enumerate(rectangles):
        for lineIndex, line in enumerate(lines):
            ...
            minValue = max(rectanglePoints[rectangle[0]][lineDim], rectanglePoints[rectangle[2 - lineDim]][lineDim],
                           linePoints[line[0]][lineDim])
            maxValue = min(rectanglePoints[rectangle[1 + lineDim]][lineDim], rectanglePoints[rectangle[3]][lineDim],
                           linePoints[line[1]][lineDim])

            if maxValue - minValue < gap:
                continue

            rectangleFixedValue_1 = (rectanglePoints[rectangle[0]][1 - lineDim] +
                                     rectanglePoints[rectangle[1 + lineDim]][1 - lineDim]) / 2
            rectangleFixedValue_2 = (rectanglePoints[rectangle[2 - lineDim]][1 - lineDim] +
...
def findConflictLinePairs(points, lines, gap, distanceThreshold, considerEndPoints=False):
    ...
    for lineIndex_1, line_1 in enumerate(lines):
        ...
        for lineIndex_2, line_2 in enumerate(lines):
            # print('line 2 index', lineIndex_2)
            ...
            if considerEndPoints:
                if min([pointDistance(points[line_1[0]], points[line_2[0]]),
                        pointDistance(points[line_1[0]], points[line_2[1]]),
                        pointDistance(points[line_1[1]], points[line_2[0]]),
                        pointDistance(points[line_1[1]], points[line_2[1]])]) <= gap:
                    conflictLinePairs.append((lineIndex_1, lineIndex_2))
                    continue
                pass
            ...
            if lineDim_1 == lineDim_2:
                if abs(
                        fixedValue_2 - fixedValue_1) >= distanceThreshold or minValue_1 > maxValue_2 - gap or minValue_2 > maxValue_1 - gap:
                    continue
...
def findConflictRectanglePairs(points, rectangles, gap):
    ...
    for rectangleIndex_1, rectangle_1 in enumerate(rectangles):
        for rectangleIndex_2, rectangle_2 in enumerate(rectangles):
            ...
            minX = max((points[rectangle_1[0]][0] + points[rectangle_1[2]][0]) / 2,
                       (points[rectangle_2[0]][0] + points[rectangle_2[2]][0]) / 2)
            maxX = min((points[rectangle_1[1]][0] + points[rectangle_1[3]][0]) / 2,
                       (points[rectangle_2[1]][0] + points[rectangle_2[3]][0]) / 2)
            if minX > maxX - gap:
                continue
            minY = max((points[rectangle_1[0]][1] + points[rectangle_1[1]][1]) / 2,
                       (points[rectangle_2[0]][1] + points[rectangle_2[1]][1]) / 2)
            maxY = min((points[rectangle_1[2]][1] + points[rectangle_1[3]][1]) / 2,
    ...
    for rectangleIndex, rectangle in enumerate(rectangles):
        for lineIndex, line in enumerate(lines):
            ...
            if lineDim == 0:
                ...
                if minX > maxX - gap:
                    continue
                if max(rectanglePoints[rectangle[0]][1], rectanglePoints[rectangle[1]][1]) + gap > min(
                        linePoints[line[0]][1], linePoints[line[1]][1]):
                    continue
                if min(rectanglePoints[rectangle[2]][1], rectanglePoints[rectangle[3]][1]) - gap < max(
                        linePoints[line[0]][1], linePoints[line[1]][1]):
                    continue
                ...
                if minY > maxY - gap:
                    continue
                if max(rectanglePoints[rectangle[0]][0], rectanglePoints[rectangle[2]][0]) + gap > min(
                        linePoints[line[0]][0], linePoints[line[1]][0]):
                    continue
                if min(rectanglePoints[rectangle[1]][0], rectanglePoints[rectangle[3]][0]) - gap < max(
                        linePoints[line[0]][0], linePoints[line[1]][0]):
                    continue
            ...
            if iconIndex <= 10:
                ...
                if sizeX >= LENGTH_THRESHOLDS['door'] and sizeY * 2 <= sizeX:
                    ...
                    if (sizeX - minOffset - maxOffset) * 2 <= sizeY and sizeX - minOffset - maxOffset > 0:
                        newDoorPoints += [[(minX + minOffset + maxX - maxOffset) / 2, minY, 0, 2],
                                          [(minX + minOffset + maxX - maxOffset) / 2, maxY, 0, 0]]
                        newDoorLines.append((doorPointOffset, doorPointOffset + 1))
                        doorPointOffset += 2
                        pass
                    ...
                    if (sizeY - minOffset - maxOffset) * 2 <= sizeX and sizeY - minOffset - maxOffset > 0:
                        newDoorPoints += [[minX, (minY + minOffset + maxY - maxOffset) / 2, 0, 1],
                                          [maxX, (minY + minOffset + maxY - maxOffset) / 2, 0, 3]]
                        newDoorLines.append((doorPointOffset, doorPointOffset + 1))
                        doorPointOffset += 2
                        pass
                    pass
...
def reconstructFloorplan(wallCornerHeatmaps, doorCornerHeatmaps, iconCornerHeatmaps, iconHeatmaps, roomHeatmaps,
                         densityImage=None, gt_dict=None, gt=False, gap=-1, distanceThreshold=-1, lengthThreshold=-1,
                         debug_prefix='test', heatmapValueThresholdWall=None, heatmapValueThresholdDoor=None,
    ...
    doorPoints, doorLines, doorPointOrientationLinesMap, doorPointNeighbors, _ = extractCorners(doorCornerHeatmaps,
                                                                                                heatmapValueThresholdDoor,
                                                                                                gap=GAPS[
                                                                                                    'door_extraction'],
                                                                                                cornerType='door',
                                                                                                gt=gt)
    iconPoints, iconLines, iconPointOrientationLinesMap, iconPointNeighbors, _ = extractCorners(iconCornerHeatmaps,
    ...
    if True:
        ...
        drawLines(os.path.join(debug_prefix, 'lines.png'), width, height, wallPoints, wallLines, [], None, 1,
                  lineColor=255)
    else:
        ...
        drawLines(os.path.join(debug_prefix, 'lines.png'), width, height, wallPoints, wallLines, [], None, 2,
                  lineColor=255)
        pass
    ...
    if not gt:
        newIcons, newIconPoints, newDoorLines, newDoorPoints = findCandidatesFromHeatmaps(iconHeatmaps, len(iconPoints),
                                                                                          len(doorPoints))
    ...
    if True:
        drawLines(os.path.join(debug_prefix, 'lines.png'), width, height, wallPoints, wallLines, [], None, 2,
                  lineColor=255)
        drawLines(os.path.join(debug_prefix, 'doors.png'), width, height, doorPoints, doorLines, [], None, 2,
                  lineColor=255)
    ...
    conflictWallLinePairs = findConflictLinePairs(wallPoints, wallLines, gap=GAPS['wall_conflict'],
                                                  distanceThreshold=DISTANCES['wall'], considerEndPoints=True)
    # print(len(wallLines))
    ...
    conflictDoorLinePairs = findConflictLinePairs(doorPoints, doorLines, gap=GAPS['door_conflict'],
    ...
    if False:
        ...
        for lineIndex, line in enumerate(wallLines):
            drawLines(os.path.join(debug_prefix, 'lines/line_' + str(lineIndex) + '.png'), width, height, wallPoints,
                      [line], [], lineColor=255)
            continue
        exit(1)
        pass
    ...
    iconWallLineNeighbors = findRectangleLineNeighbors(iconPoints, icons, wallPoints, wallLines, wallLineNeighbors,
                                                       gap=GAPS['wall_icon_neighbor'],
                                                       distanceThreshold=DISTANCES['wall_icon'])
    conflictIconWallPairs = findConflictRectangleLinePairs(iconPoints, icons, wallPoints, wallLines,
    ...
    if False:
        ...
        for lineIndex, neighbors in enumerate(wallLineNeighbors):
            if len(neighbors[0]) == 0 and len(neighbors[1]) > 0:
                ...
                pass
            continue
        drawLines(os.path.join(debug_prefix, 'exterior_1.png'), width, height, wallPoints, filteredWallLines,
                  lineColor=255)
        ...
        for lineIndex, neighbors in enumerate(wallLineNeighbors):
            if len(neighbors[0]) > 0 and len(neighbors[1]) == 0:
                ...
                pass
            continue
        drawLines(os.path.join(debug_prefix, 'exterior_2.png'), width, height, wallPoints, filteredWallLines,
                  lineColor=255)
        exit(1)
        pass
    ...
    try:
        ...
        # add variables
        w_p = [model.addVar(vtype=GRB.BINARY, name="point_" + str(pointIndex)) for pointIndex in
               xrange(len(wallPoints))]
        w_l = [model.addVar(vtype=GRB.BINARY, name="line_" + str(lineIndex)) for lineIndex in xrange(len(wallLines))]

        d_l = [model.addVar(vtype=GRB.BINARY, name="door_line_" + str(lineIndex)) for lineIndex in
               xrange(len(doorLines))]

        i_r = [model.addVar(vtype=GRB.BINARY, name="icon_rectangle_" + str(lineIndex)) for lineIndex in
               xrange(len(icons))]

        i_types = []
        for iconIndex in xrange(len(icons)):
            i_types.append(
                [model.addVar(vtype=GRB.BINARY, name="icon_type_" + str(iconIndex) + "_" + str(typeIndex)) for typeIndex
                 in xrange(NUM_FINAL_ICONS)])
            continue
        ...
        for lineIndex in xrange(len(wallLines)):
            ...
            for direction in xrange(2):
                ...
                for label in xrange(NUM_FINAL_ROOMS):
                    labels.append(model.addVar(vtype=GRB.BINARY,
                                               name="line_" + str(lineIndex) + "_" + str(direction) + "_" + str(label)))
        ...
        if False:
            ...
            for lineIndex in xrange(len(wallLines)):
                for oppositeRoomPair in oppositeRoomPairs:
                    model.addConstr(l_dir_labels[lineIndex][0][oppositeRoomPair[0]] + l_dir_labels[lineIndex][0][
                        oppositeRoomPair[1]] <= 1)
                    if oppositeRoomPair[0] != oppositeRoomPair[1]:
                        model.addConstr(l_dir_labels[lineIndex][0][oppositeRoomPair[1]] + l_dir_labels[lineIndex][0][
                            oppositeRoomPair[0]] <= 1)
                        pass
                    continue
                continue
            pass
        ...
        for lineIndex in xrange(len(wallLines)):
            ...
            label = 0
            model.addConstr(l_dir_labels[lineIndex][0][label] + l_dir_labels[lineIndex][1][label] == w_l[lineIndex],
                            'exterior_wall')
        ...
        if False:
            for lineIndex, line in enumerate(wallLines):
                ...
                for delta in xrange(int(round(abs(neighborPoint[lineDim] - point[lineDim]))) + 1):
                    ...
                    if lineDim == 0:
                        ...
                        wallEvidenceSum += roomHeatmaps[fixedValue_1:fixedValue_2,
                                           min(max(intermediatePoint[0], 0), width - 1), WALL_LABEL_OFFSET].sum()
                    else:
                        ...
                        wallEvidenceSum += roomHeatmaps[min(max(intermediatePoint[1], 0), height - 1),
                                           fixedValue_1:fixedValue_2, WALL_LABEL_OFFSET].sum()
                        pass
            ...
            for lineIndex, line in enumerate(wallLines):
                ...
                point_1[lineDim], point_2[lineDim] = min(point_1[lineDim], point_2[lineDim]), max(point_1[lineDim],
                                                                                                  point_2[lineDim])
                ...
                wallLineConfidence = np.sum(
                    wallLineConfidenceMap[point_1[1]:point_2[1] + 1, point_1[0]:point_2[0] + 1]) / (
                                                 (point_2[1] + 1 - point_1[1]) * (point_2[0] + 1 - point_1[0])) - 0.5
        ...
        if not gt:
            for wallIndex, wallLine in enumerate(wallLines):
                print(
                wallIndex, [np.array(wallPoints[pointIndex][:2]).astype(np.int32).tolist() for pointIndex in wallLine],
                wallConfidences[wallIndex])
                continue
        ...
        for lineIndex, line in enumerate(doorLines):
            ...
            if not gt:
                doorLineConfidence = np.sum(
                    doorLineConfidenceMap[point_1[1]:point_2[1] + 1, point_1[0]:point_2[0] + 1]) / (
                                                 (point_2[1] + 1 - point_1[1]) * (point_2[0] + 1 - point_1[0]))

                if lineDim == 0:
                    doorPointConfidence = (doorCornerHeatmaps[point_1[1], point_1[0], 3] + doorCornerHeatmaps[
                        point_2[1], point_2[0], 1]) / 2
                else:
                    doorPointConfidence = (doorCornerHeatmaps[point_1[1], point_1[0], 0] + doorCornerHeatmaps[
                        point_2[1], point_2[0], 2]) / 2
                    pass
        ...
        for iconIndex, icon in enumerate(icons):
            ...
            iconTypeConfidence = iconHeatmaps[y_1:y_2 + 1, x_1:x_2 + 1, :NUM_FINAL_ICONS + 1].sum(
                axis=(0, 1)) / iconArea
            ...
            if not gt:
                ...
                iconPointConfidence = (iconCornerHeatmaps[int(round(point_1[1])), int(round(point_1[0])), 2] +
                                       iconCornerHeatmaps[int(round(point_2[1])), int(round(point_2[0])), 3] +
                                       iconCornerHeatmaps[int(round(point_3[1])), int(round(point_3[0])), 1] +
                                       iconCornerHeatmaps[int(round(point_4[1])), int(round(point_4[0])), 0]) / 4 - 0.5
        ...
        if model.status == GRB.Status.OPTIMAL:
            ...
            for lineIndex, lineVar in enumerate(w_l):
                ...
                print('wall', lineIndex, labels,
                      [np.array(wallPoints[pointIndex][:2]).astype(np.int32).tolist() for pointIndex in
                       wallLines[lineIndex]], wallLineNeighbors[lineIndex][0].keys(),
            ...
            if not gt:
                ...
                adjustPoints(wallPoints, filteredWallLines)
                filteredWallLabels = [filteredWallLabels[lineIndex] for lineIndex in xrange(len(filteredWallLines)) if
                                      filteredWallLines[lineIndex][0] != filteredWallLines[lineIndex][1]]
            ...
            filteredDoorWallMap = findLineMapSingle(doorPoints, filteredDoorLines, wallPoints, filteredWallLines,
            ...
            with open('test/floorplan.txt', 'w') as result_file:
                ...
                for wallIndex, wall in enumerate(filteredWallLines):
                    ...
                    result_file.write(
                        str(filteredWallLabels[wallIndex][0]) + '\t' + str(filteredWallLabels[wallIndex][1]) + '\n')
